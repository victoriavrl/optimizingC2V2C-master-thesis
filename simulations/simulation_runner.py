...
def run_simulations(ev_paths, smart, public, oracle=False):
    ...
    for path in ev_paths:
        ...
        if smart:
            ...
            w_path = f"data_ev/workplace_data/{ev.name}_{ev.battery_capacity}_{label}_{'oracle_' if ev.oracle else ''}.csv"
            h_path = f"data_ev/home_data/{ev.name}_{ev.battery_capacity}_{label}_{'oracle_' if ev.oracle else ''}.csv"
        else:
